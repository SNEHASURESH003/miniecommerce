...
def admin_register(request):
    if not request.user.is_superuser:
        return redirect('login')  # 👈 redirect to user login if not superuser

    if request.method == 'POST':
        form = UserRegisterForm(request.POST)
        if form.is_valid():
            user = form.save(commit=False)
            user.is_staff = True  # ✅ makes them an admin/staff
            user.save()
            return redirect('admin_login')  # redirect to admin login after success
    else:
        form = UserRegisterForm()
